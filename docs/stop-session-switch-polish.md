...
| **Per-session module stream buffer** | `streamingBuffersBySession` map in `chatStreaming.ts`; append/clear/flush keyed by owning `sessionId`. Background A append/complete/Stop must not corrupt B's buffer (and vice versa). Selected `streamingContent` chrome remains owner-gated. |
