...
export function SessionFolderChip({
  ...
  const emptyHint =
    kind === 'project' ? t('chat.noProjectFolderHint') : t('chat.noPipiOutputFolderHint');
  const emptyHintTestId =
    kind === 'project' ? 'project-folder-unbound-hint' : 'pipi-output-folder-unbound-hint';

  if (!value) {
    return (
      <div className="flex flex-col gap-0.5 max-w-[260px]" data-testid={`${emptyTestId}-wrap`}>
        <button
          onClick={onBind}
          disabled={isBinding}
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full
                     ...
                     transition-all duration-150 ${
                       kind === 'project'
                         ? 'border-amber-300/80 text-amber-700/80 hover:border-amber-400 hover:text-amber-800'
                         : 'border-gray-200 text-gray-400 hover:border-gray-300 hover:text-gray-600'
                     }`}
          title={`${tooltip} — ${emptyHint}`}
          aria-description={emptyHint}
          data-testid={emptyTestId}
        >
          <span
            className={`text-[10px] font-semibold uppercase tracking-wide ${
              kind === 'project' ? 'text-amber-600/90' : 'text-gray-400'
            }`}
          >
            {label}
          </span>
        ...
        <span
          className={`text-[10px] leading-snug px-1 ${
            kind === 'project' ? 'text-amber-700/80' : 'text-gray-400'
          }`}
