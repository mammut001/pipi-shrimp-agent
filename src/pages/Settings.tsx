...
export function Settings() {
  ...
  /**
   * Save other settings (working dir, telegram, theme, language)
   */
  const handleSaveOtherSettings = async () => {
    try {
      if (otherSettings.workingDirectory !== workingDirectory) {
        await setWorkingDirectory(otherSettings.workingDirectory);
      }
      if (otherSettings.telegramToken !== (telegramToken || '')) {
        await setTelegramToken(otherSettings.telegramToken);
      }
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">Working Directory</h2>
            <input
              type="text"
              value={otherSettings.workingDirectory}
              onChange={(e) => setOtherSettings((prev) => ({ ...prev, workingDirectory: e.target.value }))}
          ...
          {/* ====== Telegram Section ====== */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">Telegram Integration</h2>
            <input
              type="password"
              value={otherSettings.telegramToken}
              onChange={(e) => setOtherSettings((prev) => ({ ...prev, telegramToken: e.target.value }))}
              placeholder="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent"
            />
            <p className="mt-1 text-xs text-gray-500">Get your bot token from @BotFather</p>
          </div>
