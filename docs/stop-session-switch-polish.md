...
| **Session-switch busy binding** | `selectSession` clears selected-session stream chrome then syncs selected session tool runtime (rebind, not idle-only); **does not** cancel/stop/scrub the previous session's in-flight tools |
| **Background completion ≠ selected chrome** | Stream completion / cancel / reasoning chrome updates only when `ownsSelectedStreamChrome(owning, current)`; A completing in background must not flip B `isStreaming` / stream buffer / busy chrome |
...
- `startSession` (new chat) still cancels previous pending tools — leave unless trivial
