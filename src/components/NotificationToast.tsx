...
function NotificationHistoryPanel({
  ...
  const handleCopyAll = () => {
    const allText = notifications
      .map((n) => `[${formatTimestamp(n.timestamp)}] ${n.message}`)
      .join('\n');
    ...
    <div className="pointer-events-auto w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="max-h-80 overflow-y-auto p-2">
        {notifications.map((notification) => (
          <div
            key={notification.id}
            className={`mb-2 flex gap-3 rounded-xl border px-3 py-2 last:mb-0 relative group ${getTypeStyles(notification.type)}`}
          >
            ...
            <button
              type="button"
              onClick={() => handleCopyIndividual(notification.id, notification.message)}
              className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-black/5 text-current"
              title="复制消息"
            >
