...
| 10 | AG-02 | **refactor: extract preflight from loopEngine.ts (PR2a)** | `loopEngine.ts` → `loopEngine.preflightPhase.ts` | `loopEngine.preflightPhase.test.ts`, `loopEngine.integration.test.ts` | Low | ✅ PR2a shipped 2026-09-17 — preflight → loopEngine.preflightPhase.ts; AG-02 overall still open — **PR2b** (iteration + metrics phases) remaining |
