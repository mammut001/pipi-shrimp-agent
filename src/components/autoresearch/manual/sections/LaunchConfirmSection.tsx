  ...
  return (
    <div className="space-y-3 font-sans">
      <div className="space-y-2 rounded-2xl border border-gray-100 bg-gray-50/70 p-3 font-sans">
        ...
        <div className="grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-2 text-xs font-sans">
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryRuntimeTarget')}
            value={formatRuntimeTargetSummary(setupForm, locale)}
          />
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryWorkspaceDir')}
            value={setupForm.remoteWorkDir || '—'}
          />
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryTargetProject')}
            value={experimentDir || '—'}
          />
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryPrimaryMetric')}
            value={formatMetricIterationsSummary({ metric, direction, baselineInput, maxIter }, locale)}
          />
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryModelApi')}
            value={providerReady ? t('autoresearch.manual.modelConfigured') : t('autoresearch.manual.modelNotConfigured')}
          />
          <AutoResearchSummaryItem
            label={t('autoresearch.manual.summaryEnvCheck')}
            value={
              connectionTestStatus === 'success'
                ? t('autoresearch.manual.envCheckPassed')
                : connectionTestStatus === 'error'
                ? t('autoresearch.manual.envCheckFailed')
                : t('autoresearch.manual.envCheckNotTested')
            }
          />
        </div>
      </div>
      <div className="flex items-center gap-2 border-t border-gray-100 pt-3 text-xs text-gray-500 font-sans">
        ...
        <span>{t('autoresearch.manual.launchReadyHint')}</span>
      </div>
    </div>
  );
}
