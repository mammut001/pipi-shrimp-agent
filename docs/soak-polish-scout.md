...
|----------|---------------|------------------------|
| **Truncated replies** | Smoke `reply only ping-ok` → **INCONCLUSIVE (provider truncation)** — app often returned `ping` only; external `curl` got full `ping-ok` (`pr73_stabilization_report.md`) | Provider/stream path: DeepSeek thinking-disable / reasoning passback commits noted in same report; stream finalize coverage in `src-tauri/src/claude/http/__tests__/integration_test.rs` (`send_request_finalizes_truncated_openai_stream`). Product check: short prompt → assert full expected token sequence in UI. |
