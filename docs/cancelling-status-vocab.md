...
| **Late tool-complete/error no-op** | `resolveSessionTool` no-ops when step is already `cancelled`; pendingToolResults stay empty; Stop stays hidden (`already_finished`/`not_found` → `done` unchanged) |
