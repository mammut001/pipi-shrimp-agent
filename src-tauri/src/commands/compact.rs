...
pub fn microcompact_clear_old_tool_results(
    ...
    for &idx in to_clear {
        ...
        database::save_message(&msg_clone).map_err(|e| format!("Failed to save: {}", e))?;
    ...
    println!(
        "🧹 Microcompact(time): cleared {} tool results (kept {} recent, {}min idle)",
        updates.len(),
        keep_count,
        idle_minutes
    );
...
pub fn microcompact_by_count(
    ...
    println!(
        "🧹 Microcompact(count): cleared {} tool results (max={}, keep={})",
        updates.len(),
        max_tool_results,
        keep_count
    );
...
pub fn get_session_token_stats(
    ...
    let sm_threshold = config
        .get("sm_auto_threshold_tokens")
        .and_then(|v| v.as_i64())
        .unwrap_or(80_000) as usize;

    let legacy_threshold = config
        .get("legacy_auto_threshold_tokens")
        .and_then(|v| v.as_i64())
        .unwrap_or(120_000) as usize;

    let warning_buffer = config
        .get("warning_buffer_tokens")
        .and_then(|v| v.as_i64())
        .unwrap_or(20_000) as usize;

    let blocking_buffer = config
        .get("legacy_keep_tokens")
        .and_then(|v| v.as_i64())
