...
| Related tests | `src/store/__tests__/setSessionWorkDirFromPath.test.ts` (asserts `init_pipi_shrimp` runs against the PiPi Output Folder, not the Project Folder). `src/services/prompt/__tests__/defaultTemplate.test.ts` — see "defaultTemplate PiPi Output Folder section". Artifact path validation uses canonical/real paths for existing files when available; future generated output paths must remain lexically and parent-canonically inside `workDir`/`outputDir`. |
