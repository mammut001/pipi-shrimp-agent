/**
 * Presentational section components extracted from AgentPanel.tsx.
 * Behavior-preserving extract (split-soon / <500 follow-up PR2).
 */
...
import type { CdpStatus, BrowserConnectionStatePayload } from '@/store/browser/browserConnection';
import {
...
export interface AgentPanelTabBarProps {
  activeTab: string;
  setActiveTab: (tab: 'main' | 'browser' | 'artifact-preview' | 'goal') => void;
  currentArtifactId?: string | null;
}

/**
 * Top tab bar (Main / Browser / Artifact Preview / Goal buttons)
 */
export const AgentPanelTabBar: React.FC<AgentPanelTabBarProps> = ({
  activeTab,
  setActiveTab,
  currentArtifactId,
}) => {
...
export interface AgentPanelProgressSectionProps {
  taskProgress: TaskStep[];
  onCancelToolExecution: (stepId: string, executionId: string) => void;
}

/**
 * Progress Section JSX (uses Section, taskStepDotClassName, taskStepLabelClassName,
 * ThinkingPulse, CancellingPulse, coerceRenderableText, formatCancelInterruptLabel)
 */
export const AgentPanelProgressSection: React.FC<AgentPanelProgressSectionProps> = ({
  taskProgress,
        ...
        <div className="space-y-3 pt-2">
          {taskProgress.map((step, idx) => (
            <div key={step.id} className="flex gap-3 items-start relative group">
              ...
              <div
                className={`mt-0.5 h-4.5 w-4.5 rounded-full flex items-center justify-center flex-shrink-0 z-10 transition-all ${taskStepDotClassName(
                  step.status,
                )}`}
              >
                {step.status === 'done' ? (
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <p
                    className={`min-w-0 flex-1 text-[11px] font-medium leading-[1.4] transition-colors ${taskStepLabelClassName(
                      step.status,
                    )}`}
                  >
                ...
                {step.status === 'cancelled' && (
                  <p className="mt-1 text-[9px] font-bold uppercase tracking-tight text-slate-500">
                    {formatCancelInterruptLabel('cancelled')}
                  </p>
                )}
...
export interface AgentPanelWorkingFoldersSectionProps {
  syncedFiles: SyncedWorkspaceEntry[];
  allWorkingFiles: ImportedFile[];
  sessionWorkingFiles: ImportedFile[];
  globalImportedFiles: ImportedFile[];
  currentSessionId?: string | null;
  onRemoveSessionWorkingFile: (sessionId: string, fileId: string) => void;
  onRemoveImportedFile: (fileId: string) => void;
  onClearImportedFiles: () => void;
  onRevealInFinder?: (path: string) => void;
}

/**
 * Working Folders Section (uses Section, FileIcon, workingFoldersCountBadge,
 * i18n keys, data-testid="working-folders-empty", reveal_in_finder)
 */
export const AgentPanelWorkingFoldersSection: React.FC<AgentPanelWorkingFoldersSectionProps> = ({
  syncedFiles,
  allWorkingFiles,
  sessionWorkingFiles,
  globalImportedFiles,
  currentSessionId,
  onRemoveSessionWorkingFile,
          ...
          allWorkingFiles.map((file) => {
            ...
            const handleRemove = () => {
              if (isSessionFile && currentSessionId) {
                onRemoveSessionWorkingFile(currentSessionId, file.id);
              } else {
                onRemoveImportedFile(file.id);
              }
            };
            return (
              <div
                key={file.id}
                className="group flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100/50 rounded-lg transition-all"
              >
                <FileIcon filename={file.name} />
                <span className="flex-1 text-[11px] text-gray-700 truncate font-medium" title={file.path}>
                  {file.name}
...
export interface AgentPanelContextSectionProps {
  activeSkill: string | null;
  coreSkills: SkillInfo[];
  remainingCount: number;
  cdpStatus: CdpStatus;
  cdpConnectionState?: BrowserConnectionStatePayload | null;
  onOpenConnectorModal: () => void;
  agentInstructions: string;
  localInstructions: string;
  onChangeLocalInstructions: (value: string) => void;
  onSaveSoul: () => void;
  isSavingSoul: boolean;
}

/**
 * Context Section with Skills + Connectors + Agent Soul textarea
 */
export const AgentPanelContextSection: React.FC<AgentPanelContextSectionProps> = ({
  activeSkill,
  coreSkills,
  remainingCount,
  cdpStatus,
...
export interface AgentPanelFooterProps {
  taskProgress: Array<{ status: string }>;
  version?: string;
}
 ...
 * Footer / status area
 */
export const AgentPanelFooter: React.FC<AgentPanelFooterProps> = ({
  taskProgress,
      ...
      <div className="flex items-center gap-2">
        <div
          className={`h-1.5 w-1.5 rounded-full ${
            taskProgress.some((s) => s.status === 'running' || s.status === 'cancelling')
...
export interface AgentPanelBrowserTabProps {}

/**
 * Browser tab content wrapper
 */
export const AgentPanelBrowserTab: React.FC<AgentPanelBrowserTabProps> = () => {
  return (
...
export interface AgentPanelArtifactTabProps {
  artifactId?: string;
  messages: any[];
}

/**
 * Artifact preview tab content wrapper
 */
export const AgentPanelArtifactTab: React.FC<AgentPanelArtifactTabProps> = ({
  artifactId,
  messages,
}) => {
  return (
    <div className="flex-1 overflow-hidden p-3">
      <ArtifactRenderer artifactId={artifactId} messages={messages} />
    </div>
  );
};
