...
| R2-11 | MCP stdio cwd unsandboxed | High | `mcp/transport/stdio.rs` | validate cwd via path_security **fixed** 2026-09-21 | `test_cwd_sandbox` | No |
| R2-12 | `mcp_call_tool` no policy | High | `commands/mcp.rs` | Policy wrapper via execution_policy **fixed** 2026-09-21 | `test_destructive_requires_approval` | No |
...
LOC column = live `wc -l` on `main` (`b6c811a`, 2026-09-24). Original audit baselines were all >800; open rows are now under the 800 hard limit but remain split-soon (500–800) or >500 component. Per [complexity-governance.md](../architecture/complexity-governance.md) §1–§2, `split soon` (500–800 LOC) means "plan a split in the current milestone; do not add new features on top", and a `.tsx` component >500 LOC is in the component `refactor` band — so every open row below still blocks **new feature work** on that file until it is split (bug fixes / refactors allowed).
...
| AG-10 | `src-tauri/src/commands/web.rs` | **397** | **Fixed** — #196 extracted shared action helpers into `web/action_helpers.rs` (131; `clone_manager_handle`/`action_context`/`action_result` + `*_with_ctx` wrappers, verbatim) next to existing `web/cdp.rs` (CDP bodies + R3-06 policy gate) / `web/search.rs` (514→397; under 500; all 34 `#[tauri::command]` fns stay in `web.rs`, `generate_handler!` paths + await points + error strings + serde attrs unchanged) (2026-09-24) | No | Done — #196 |
| AG-11 | `src/store/workflowStore.ts` | **362** | **Fixed** — #194 extracted zustand action slices `workflowAgentActions.ts` (274; agent CRUD/status/dirty) + `workflowConnectionActions.ts` (158; connections + output routes), spread at the original position alongside existing `workflowRunActions.ts` (712→362; under 500; exported API unchanged; all actions still synchronous) (2026-09-24) | No | Done — #194 |
| AG-12 | `src/services/autoresearch/chatAdapter.ts` | **418** | **Fixed** — #192 extracted `chatAdapterReflectionEvents.ts` (123) + `chatAdapterToolHooks.ts` (234) + `chatAdapterReasoning.ts` (50) + pure `chatAdapterRecoveryPrompt.ts` (100) (762→418; under 500; exported API unchanged) (2026-09-24) | No | Done — #192 |
...
| AG-14 | `src/pages/Settings.tsx` | **459** | **Fixed** — #191 extracted display-only `settings/settingsSections.tsx` (141) + pure `settings/settingsFormHelpers.ts` (210) (729→459; under 500 component threshold) (2026-09-24) | No | Done — #191 |
...
| AG-23 | `src-tauri/src/commands/code.rs` | **678** | **Open — careful (security-sensitive).** Progress ✅ #203: tests moved to `code/tests.rs`. Progress ✅ #210: Python session pool/implementation → `code/python_session.rs`, cwd/WSL helpers → `code/cwd.rs` (1071→678, under 800; command wrappers and safety logic stay). Next: continue mechanical extraction to <500 without moving safety decisions | Yes | Yes |
...
| AG-27 | `src/pages/AutoResearch.tsx` | **761** | **Open.** Progress ✅ #206: `loadPersistedSetup` + types → `autoResearch/autoResearchSetupPersistence.ts`, `ExperimentDetailPanel` → own file, and display-only sections `AutoResearchRunControls` / `AutoResearchSetupFields` / `AutoResearchEmptyState` / `AutoResearchRunListView` (1131→761, under 800; handlers/hooks stay in the page, 47+3 hooks unchanged). Next for <500: more AutoResearchView JSX sections the same way | Yes | Yes |
