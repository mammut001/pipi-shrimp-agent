  ...
  return (
    <section className={`rounded-[16px] border border-white/10 bg-[#1a1a1a] p-4 ${className}`}>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/40">Metric History</p>
          <p className="mt-2 text-sm text-white/55">
            {run.config.metric || 'metric'} · {run.config.direction === 'lower' ? 'lower is better' : 'higher is better'}
          </p>
        </div>
      </div>
