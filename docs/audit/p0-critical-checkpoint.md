...
| 10 | AG-02 | **refactor: extract preflight from loopEngine.ts** | `loopEngine.ts` → `loopEngine.preflightPhase.ts` | `loopEngine.preflightPhase.test.ts`, `loopEngine.integration.test.ts` | Low | ✅ Shipped 2026-09-17 — preflight extracted to loopEngine.preflightPhase.ts, dead buildDirtyRepoMessage removed, R5-02 try/finally preserved |
