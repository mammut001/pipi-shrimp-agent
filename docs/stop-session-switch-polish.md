...
| **Permission approval isolation** | Pending `waitForPermission` promises are **session-scoped**; `startSession` / `selectSession` must **not** `clearAllPermissions` (that denies other sessions). UI shows only the selected session's queue entry. Stop / delete still clears the owning session via `clearPermissionsForSession`. Mode updates (`updateSessionPermissionMode` / `updateSessionExecutionMode`) settle **only** the owning `sessionId` queue entries, once (no cross-session resolve / no double-settle). |
