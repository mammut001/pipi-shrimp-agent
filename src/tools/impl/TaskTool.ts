...
export class TaskCreateTool extends BaseTool<TaskCreateInput, TaskCreateOutput> {
  ...
  async execute(input: TaskCreateInput, _context: ToolContext): Promise<ToolResult<TaskCreateOutput>> {
    const task = taskStore.create(input.subject, input.description, input.metadata);
    return { success: true, data: { taskId: task.id, subject: task.subject } };
  }
}
...
export class TaskGetTool extends BaseTool<TaskGetInput, TaskGetOutput> {
  ...
  async execute(input: TaskGetInput, _context: ToolContext): Promise<ToolResult<TaskGetOutput>> {
...
export class TaskListTool extends BaseTool<TaskListInput, TaskListOutput> {
  ...
  async execute(input: TaskListInput, _context: ToolContext): Promise<ToolResult<TaskListOutput>> {
...
export class TaskUpdateTool extends BaseTool<TaskUpdateInput, TaskUpdateOutput> {
  ...
  async execute(input: TaskUpdateInput, _context: ToolContext): Promise<ToolResult<TaskUpdateOutput>> {
    ...
    return {
      ...
      data: {
        success: true,
        taskId: task.id,
        updatedFields: Object.keys(input).filter((key) => key !== 'taskId'),
      },
    };
  }
}
...
export class TaskStopTool extends BaseTool<TaskStopInput, TaskStopOutput> {
  ...
  async execute(input: TaskStopInput, _context: ToolContext): Promise<ToolResult<TaskStopOutput>> {
...
export class TaskOutputTool extends BaseTool<TaskOutputInput, TaskOutputOutput> {
  ...
  async execute(input: TaskOutputInput, _context: ToolContext): Promise<ToolResult<TaskOutputOutput>> {
    const task = taskStore.get(input.taskId);
    return {
      success: true,
      data: {
